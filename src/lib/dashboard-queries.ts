...
import { format, eachMonthOfInterval, startOfMonth, endOfMonth, subDays, startOfDay } from "date-fns";
...
export function useQuickStats() {
  return useQuery({
    ...
    staleTime: STALE,
    queryFn: async () => {
      const [inv, cust, sup, salesRpc] = await Promise.all([
        supabase.from("invoices").select("id", { count: "exact", head: true }),
        supabase.from("customers").select("id", { count: "exact", head: true }),
        supabase.from("suppliers").select("id", { count: "exact", head: true }),
        supabase.rpc("sum_invoices_by_type", {
          p_type: "sales",
          p_from: "2020-01-01",
      ...
      const totalSales = Number(salesRpc.data?.[0]?.total ?? 0);
      ...
      const pendingAmt = (pendingData ?? []).reduce((s: number, r: any) => s + Number(r.total_amount ?? 0), 0);

      return {
        totalInvoices,
        totalCustomers: cust.count ?? 0,
        totalSuppliers: sup.count ?? 0,
        avgInvoice: totalInvoices ? totalSales / totalInvoices : 0,
